<template>
	<div class="body-content diy-content">
		...
		<div class="diy-list" v-infinite-scroll="loadMore" infinite-scroll-disabled="loading" infinite-scroll-distance="10">
			...
			<ul v-for="(item, key) in months" :key="key" :data-name="chineseMonth[item.month] + '月'">
				...
				<li v-for="(day, i) in item.days" 
					:key="i" :style="{marginLeft: day === 1 ? getFirstMarginLeft(item.year, item.month) : 0}" 
					:class="isWeekEnd(item.year, item.month, day) && 'red'" @click="selectday(item.year, item.month, day)">{{day}}
...
<script>
	...
	export default {
		...
		methods: {
			...
			// 选择天
			selectday(y, m, d) {
				if(this.diyArry.length==2){
					this.diyArry = [];
				}
				let date = y+"-"+m+"-"+d;
				this.diyArry.push(date)
				if(this.diyArry.length==2){
					this.$emit('onSelectDate', this.diyArry);
