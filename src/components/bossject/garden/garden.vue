<template>
	<div class="wapper-garden">
		<div class="gardenHeader">
			<yd-button class="headDate" @click.native="gardenPupup" :status="false">{{MessDate}}<i class="triangle_border_bottom"></i></yd-button>
			<div class="headMidle">
...
<script>
	...
	import { getGarden } from 'src/service/getData'
	...
	export default {
		...
		methods: {
			...
			async getLyMethod(date) {
				...
				let getLydata = await getGarden(starttime, endtime);
				if(getLydata.code == ApiConst.ERROR_SUCCESS) {
					...
					_self.gardenlist = getYearData.list;
				}
			},
			previousMouse() {
...
<style>
	@import "../../../assets/css/bossject/garden.css";
</style>