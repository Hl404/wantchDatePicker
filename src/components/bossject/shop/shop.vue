<template>
	<div class="wapper-shop">
		<div class="shopHeader">
			<yd-button class="headDate" @click.native="shopPupup" :status="false">{{MessDate}}<i class="triangle_border_bottom"></i></yd-button>
			<div class="headMidle">
				...
				<table>
					...
					<tbody>
						<tr v-for="item in shoplist">
							<td>{{item.name}}</td>
							<td>{{item.num}}</td>
							<td v-if="item.source=='offline'">线下</td>
							<td v-if="item.source=='online'">线上</td>
...
<script>
	...
	import { getGood } from 'src/service/getData'
	...
	export default {
		...
		methods: {
			...
			async getLyMethod(date) {
				...
				let getLydata = await getGood(starttime, endtime);
				if(getLydata.code == ApiConst.ERROR_SUCCESS) {
					...
					_self.shoplist = getYearData.list;
				}
			},
			previousMouse() {
...
<style>
	@import "../../../assets/css/bossject/shop.css";
</style>